function(input, output, session){
  ...
  #--------------------------------------------------------------------------------------------------------------------------------------
    ...
    LA_df_task2 <- reactive({
      
      n1 <- length(which(df_data()$Task2==1))
      n0 <- input$enrollement - n1
    ...
    output$dis_task2 <- renderPlot({
      ggplot(LA_df_task2()[[1]], aes(x, y))+
        geom_line()+
        labs(x=bquote(~ theta), y = "Density", title = "Posterior")+
        geom_vline(xintercept=LA_df_task2()[[2]], size=1.5, color="red")+
        geom_text(aes(x=LA_df_task2()[[2]]+0.09, label=paste0("Mean:",LA_df_task2()[[2]]), y=10))+
        theme_light()+
        theme(axis.title.x = element_text(size = 20))+
        theme(plot.title = element_text(hjust = 0.5))
    })
    ...
    output$prior_plot_task2 <- renderPlot({
      ggplot(LA_data_density(), aes(x, y))+
        geom_line()+
        labs(x=bquote(~ theta), y = "Density", title = "Encoding Our Belief:Prior")+
        theme_light()+
